...
export class CarouselTool {
  ...
  _createItem(active = false, url, captionData) {
    ...
    let caption = this._createCaption(captionData);
  ...
  _createCaption(captionData) {
    let caption = document.createElement("div");
    caption.classList.add("carousel-caption");

    let header = document.createElement("h5");
    header.setAttribute("contentEditable", "true");
    header.innerHTML =  captionData.header || "Заголовок";

    let text = document.createElement("p");
    text.setAttribute("contentEditable", "true");
    text.innerHTML = captionData.text || "Текст";

    caption.appendChild(header);
    caption.appendChild(text);

    Promise.resolve().then( () => {
      if(captionData.button)
      {
        this.FileUploader.addATag(captionData.button);
      }
    });
